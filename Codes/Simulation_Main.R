...
suppressPackageStartupMessages(library("GMPR")) # GMPR
...
normParams<-c('CSS', 'CLR', 'RLE', 'TMM', 'TSS', 'GMPR')
...
if (normMethod=='RFA') {
  RFAlist<-list.RFA(simlist.filtered)
  names(RFAlist) <- names(simlist.filtered)
  fitlist<-RFAlist
}
...
if (normMethod=='TSS') {
  TSSlist<-list.TSS(simlist.filtered)
  names(TSSlist) <- names(simlist.filtered)
  fitlist<-TSSlist
}
...
#####################
if (normMethod=='CLR') {
  CLRlist<-list.CLR(simlist.filtered)
  names(CLRlist) <- names(simlist.filtered)
  fitlist<-CLRlist
}
...
if (normMethod=='CSS') {
  CSSlist<-list.CSS(simlist.filtered)
  names(CSSlist) <-  names(simlist.filtered)
  fitlist<-CSSlist
}
...
if (normMethod=='TMM') {
  TMMlist<-list.TMM(simlist.filtered)
  names(TMMlist) <-  names(simlist.filtered)
  fitlist<-TMMlist
}
...
if (normMethod=='RLE') {
  RLElist<-list.RLE(simlist.filtered)
  names(RLElist) <-  names(simlist.filtered)
  fitlist<-RLElist
}
...
if (normMethod=='GMPR') {
  GMPRlist<-list.GMPR(simlist.filtered)
  names(GMPRlist) <-  names(simlist.filtered)
  fitlist<-GMPRlist
}
